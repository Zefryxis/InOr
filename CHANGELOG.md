...
- Randomised the scroll-move cooldown gap (was a fixed, metronomic interval — a known auto-clicker tell).
- Added missing per-player rate limits to the warehouse link-management packets (previously unthrottled, could force repeated disk writes).
- Added a low-frequency self-heal sweep for an internal borrowed-ingredient bookkeeping ledger (defensive only, never touches saved settings), and removed a leftover unused network payload.

### Localization
- Localized ~96 previously hardcoded, English-only UI strings across all 9 languages (Kits, Custom Group editor/list, Tier/Sorting Order config, the Inventory config screen, the Remote Crafting panel, Item Names, and a few screen titles — including several full "Guide" help-text blocks that were never wired into the language system).

### Tutorial
- Pressing Enter now skips the Tutorial's typewriter caption animation instead of waiting it out.
