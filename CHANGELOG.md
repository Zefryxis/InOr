...
- Security/anti-cheat pass: randomised the scroll-move cooldown gap (was a fixed, metronomic interval — a known auto-clicker tell), added missing per-player rate limits to the warehouse link-management packets (previously unthrottled, could force repeated disk writes), added a low-frequency self-heal sweep for an internal borrowed-ingredient bookkeeping ledger (defensive only, never touches saved settings), and removed a leftover unused network payload.
