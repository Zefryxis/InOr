...
public class InventoryOrganizerClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        // --- One-time migration: adopt the research-backed sort defaults (enchant priority order,
        // copper material position) for existing users, not just fresh installs. ---
        com.example.inventoryorganizer.config.OrganizerConfig.get().applySortDefaultsOnce();
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            // --- Auto-refill + trash (free mode). Throttled to every 3rd tick (~150 ms); both already
            // self-cooldown at 120/100 ms so this keeps the same responsiveness with far fewer idle scans.
            if (++refillTrashTick % 3 == 0) {
            ...
            while (REFILL_TOGGLE_KEY.consumeClick()) {
                ...
                LOGGER.info("[Refill] toggle -> {} | markedSlots={} | freeMode={} | screenOpen={}",
                        on ? "ON" : "OFF", marked, ServerEnvironment.canUseFree(), client.gui.screen() != null);
            ...
            while (WAREHOUSE_KEY.consumeClick()) {
                if (!typing && WarehouseClient.isAvailable()) {
                    client.gui.setScreen(new com.example.inventoryorganizer.warehouse.WarehouseMapScreen(client.gui.screen()));
                }
            }
            ...
            if (client.gui.screen() != null) {
                ...
                if (client.gui.screen().width != gw || client.gui.screen().height != gh) {
                    try { client.gui.screen().resize(gw, gh); } catch (Throwable ignored) {}
                }
            }
            ...
            if (client.gui.screen() instanceof net.minecraft.client.gui.screens.inventory.AbstractContainerScreen<?> acs) {
                try {
            ...
            if (client.gui.screen() instanceof net.minecraft.client.gui.screens.inventory.CraftingScreen
                    || client.gui.screen() instanceof InventoryScreen) {
                boolean needHeal = craftPanel == null || craftPanel.screen() != client.gui.screen() || !craftPanel.isAttached();
                if (WarehouseClient.isAvailable() && needHeal) {
                    try {
                        client.gui.screen().resize(client.getWindow().getGuiScaledWidth(),
        ...
        ScreenEvents.AFTER_INIT.register((client, screen, scaledWidth, scaledHeight) -> {
            ...
            Button profilesBtn = Button.builder(
                ...
                button -> {
                    ...
                    mc.gui.setScreen(new com.example.inventoryorganizer.config.ChestProfileListScreen(null, cname, cpos, csize, csign));
                }
            ...
            if (com.example.inventoryorganizer.warehouse.WarehouseClient.isAvailable()) {
                Button whBtn = Button.builder(
                    ...
                    button -> {
                        Minecraft mc = Minecraft.getInstance();
                        if (mc.player != null) mc.player.closeContainer();
                        mc.gui.setScreen(new com.example.inventoryorganizer.warehouse.WarehouseMapScreen(null));
                    }
        ...
        // --- Shulker boxes -------------------------------------------------------------------------
        // ShulkerBoxScreen is NOT a ContainerScreen, so it would fall through to the generic modded
        // handler below. We give it its own full-featured path instead: same profile UI as vanilla
        // chests, no genericChest registration (coordinates change every time a shulker moves), and
        // UUID-based profile identity (assigned by the server on open, via ShulkerIdPayload).
        ScreenEvents.AFTER_INIT.register((client, screen, sw, sh) -> {
            if (!(screen instanceof net.minecraft.client.gui.screens.inventory.ShulkerBoxScreen)) return;
            ...
            final int rows = 3; // shulker always has 3 rows
            final int chestSize = rows * 9;
            int bgHeight = 114 + rows * 18;
            int guiW = 176;
            int btnX = (screen.width - guiW) / 2 + guiW - 30;
            int panelX = (screen.width - guiW) / 2;
            ...
            // Resolve UUID-based profile (pendingShulkerId consumed here).
            ...
            // OST button
            Button ostButton = Button.builder(Component.translatable("inventory-organizer.button.ost"), b -> {
                Minecraft mc = Minecraft.getInstance();
                if (mc.player != null) StorageSorter.sortContainer(chestSize, mc.player.containerMenu.containerId);
            }).bounds(btnX, btnY, 28, 14).build();
            net.fabricmc.fabric.api.client.screen.v1.Screens.getWidgets(screen).add(ostButton);

            // OI button
            Button oiButton = Button.builder(Component.translatable("inventory-organizer.button.oi"), b -> {
                Minecraft mc = Minecraft.getInstance();
                if (mc.player == null) return;
                if (FightModeTracker.isActive() && !FightModeTracker.canUseOI()) return;
                mc.player.closeContainer();
            ...
            // Deposit button
            Button depButton = Button.builder(Component.translatable("inventory-organizer.button.deposit"),
                    b -> doDepositAllMatching(Minecraft.getInstance())
            ).bounds(btnX - 106, btnY, 24, 14).build();
            net.fabricmc.fabric.api.client.screen.v1.Screens.getWidgets(screen).add(depButton);

            // Refill button
            Button refillButton = Button.builder(Component.translatable("inventory-organizer.button.refill"),
                    b -> doChestRefill(Minecraft.getInstance())
            ).bounds(btnX - 134, btnY, 22, 14).build();
            net.fabricmc.fabric.api.client.screen.v1.Screens.getWidgets(screen).add(refillButton);

            // Profiles button
            Button profilesBtn = Button.builder(Component.translatable("inventory-organizer.profile.profiles_btn_short"), b -> {
                Minecraft mc = Minecraft.getInstance();
                String cname = ChestIdentifier.getCurrentCustomName();
                java.util.List<int[]> cpos = ChestIdentifier.getCurrentPositions();
                int csize = ChestIdentifier.getCurrentSize();
            ...
            // No Wh button for shulkers: they are portable and not part of the warehouse map.

            // Profile label + Yes/No prompt (same as vanilla chest path)
            final net.minecraft.client.gui.components.StringWidget profileLabel =
                    new net.minecraft.client.gui.components.StringWidget(panelX, panelY - 31, 110, 10, Component.empty(), client.font);
            final Runnable[] refresh = new Runnable[1];
            Button yesBtn = Button.builder(Component.translatable("inventory-organizer.profile.new_yes"),
                    b -> { ChestIdentifier.createAndBindProfile(); if (refresh[0] != null) refresh[0].run(); }
            ).bounds(panelX + 90, panelY - 32, 30, 12).build();
            Button noBtn = Button.builder(Component.translatable("inventory-organizer.profile.new_no"),
                    b -> { ChestIdentifier.markCurrentChestAsNothing(); if (refresh[0] != null) refresh[0].run(); }
            ).bounds(panelX + 122, panelY - 32, 50, 12).build();
            refresh[0] = () -> {
                StoragePreset act = ChestIdentifier.getActiveProfile();
        ...
        ScreenEvents.AFTER_INIT.register((client, screen, sw, sh) -> {
            ...
            if (screen instanceof net.minecraft.client.gui.screens.inventory.ShulkerBoxScreen) return; // shulker: handled above
        ...
        ScreenEvents.AFTER_INIT.register((client, screen, w, h) -> {
            net.fabricmc.fabric.api.client.screen.v1.ScreenKeyboardEvents.allowKeyPress(screen).register(
                (s, ev) -> {
                    // Never steal keybinds while the user is typing in a text field
                    // (chat, recipe-book search, anvil rename, sign editor, etc.).
                    if (s.getFocused() instanceof net.minecraft.client.gui.components.EditBox) return true;
                    OrganizerConfig cfg = OrganizerConfig.get();
    ...
    private static void handleSortKeybind(Minecraft client, String mode, String action) {
        ...
        boolean chestOpen = client.gui.screen() instanceof ContainerScreen;
        switch (action) {
            case "oi_only":
                doOI(client, mode);
                break;
    ...
    // Both auto-refill and trash scan all 36 slots, but each already self-throttles via a time-based
    // cooldown (120 ms / 100 ms). Running their scan on every client tick (50 ms) therefore only burns
    // CPU re-scanning the inventory while idle. We gate both calls to every 3rd tick (~150 ms), which is
    // at/above their own cooldowns, so responsiveness is unchanged while ~2/3 of idle scans are skipped.
    private static int refillTrashTick = 0;
    ...
    private static void tickAutoRefill(Minecraft client) {
        ...
        // Pass 1: update last-seen item for every non-empty refill slot; collect empty ones.
        java.util.List<Integer> emptyRefillSlots = new java.util.ArrayList<>();
        for (int slot = 0; slot < 36; slot++) {
            if (!cfg.isSlotRefill(slot)) { refillLastItem[slot] = null; continue; }
            ItemStack here = inv.getItem(slot);
            if (!here.isEmpty()) { refillLastItem[slot] = here.getItem(); continue; }
            emptyRefillSlots.add(slot);
        }

        // Pass 2: sort empty slots by tier priority (lower tier number = higher priority = fill first).
        // Among equal tier, lower slot index wins (stable, matches loop order). If two slots share
        // the same rule and the same tier, one wins deterministically (not truly random, but fair).
        emptyRefillSlots.sort((a, b) -> {
            int ta = slotTier(cfg, a), tb = slotTier(cfg, b);
            if (ta != tb) return Integer.compare(ta, tb); // tier 1 best → sort ascending
            return Integer.compare(a, b);
        });

        // Pass 3: fill the highest-priority slot that has a matching source available.
        for (int slot : emptyRefillSlots) {
    ...
    private static boolean tryDirectionalScroll(Minecraft client,
        ...
        if (!(screen instanceof InventoryScreen) && up) {
            int chestSize = size - 36;
            ItemStack toDeposit = menu.getSlot(target).getItem();
            int destSlot = findOSTSlotForDeposit(menu, chestSize, toDeposit);
            if (destSlot >= 0) {
                client.gameMode.handleContainerInput(syncId, target, 0,
                        net.minecraft.world.inventory.ContainerInput.PICKUP, client.player);
                client.gameMode.handleContainerInput(syncId, destSlot, 0,
                        net.minecraft.world.inventory.ContainerInput.PICKUP, client.player);
                if (!client.player.containerMenu.getCarried().isEmpty()) {
                    // Dest slot was occupied — put the item back and fall through to QUICK_MOVE
                    client.gameMode.handleContainerInput(syncId, target, 0,
                            net.minecraft.world.inventory.ContainerInput.PICKUP, client.player);
                } else {
                    return true;
    ...
    private static void doChestRefill(Minecraft client) {
        if (client.player == null || !(client.gui.screen() instanceof ContainerScreen)) return;
    ...
    private static void doDepositAllMatching(Minecraft client) {
        ...
        int syncId = menu.containerId;
        // Move each player slot whose item the chest already holds, using OST-aware placement where possible.
        for (int ps = chestSize; ps < total; ps++) {
            ItemStack s = menu.getSlot(ps).getItem();
            if (s.isEmpty() || !inChest.contains(s.getItem())) continue;
            int destSlot = findOSTSlotForDeposit(menu, chestSize, s);
            if (destSlot >= 0) {
                ...
                client.gameMode.handleContainerInput(syncId, destSlot, 0,
                        net.minecraft.world.inventory.ContainerInput.PICKUP, client.player);
                if (!menu.getCarried().isEmpty()) {
                    // Dest occupied — put back and use vanilla QUICK_MOVE
                    client.gameMode.handleContainerInput(syncId, ps, 0,
                            net.minecraft.world.inventory.ContainerInput.PICKUP, client.player);
                    client.gameMode.handleContainerInput(syncId, ps, 0,
                            net.minecraft.world.inventory.ContainerInput.QUICK_MOVE, client.player);
    ...
    /**
     * Find the best chest slot to OST-deposit {@code stack} into: an empty slot whose rule
     * specifically matches the item (not "any"/"empty"). Returns -1 if none found; the caller
     * should fall back to vanilla QUICK_MOVE (which handles merges and "any" slots correctly).
     */
    private static int findOSTSlotForDeposit(AbstractContainerMenu menu, int chestSize, ItemStack stack) {
        com.example.inventoryorganizer.config.StoragePreset profile = ChestIdentifier.getActiveProfile();
        if (profile == null) return -1;
        for (int i = 0; i < chestSize; i++) {
            if (!menu.getSlot(i).getItem().isEmpty()) continue;
            String rule = profile.getSlotRule(i);
            if (rule.isEmpty() || rule.equals("any") || rule.equals("empty")) continue;
            if (ruleMatchesItem(rule, stack)) return i;
        }
        return -1;
    }
    ...
    private static boolean refillTrashScreenOk(Minecraft client) {
        net.minecraft.client.gui.screens.Screen s = client.gui.screen();
        return s == null
    ...
    private static void doCyclePreset(Minecraft client) {
        ...
        if (kits.isEmpty()) {
            if (client.gui != null) client.gui.hud.setOverlayMessage(Component.translatable("inventory-organizer.preset.none"), false);
            return;
        }
        ...
        cfg.save();
        if (client.gui != null) client.gui.hud.setOverlayMessage(Component.translatable("inventory-organizer.preset.loaded", kit.getName()), false);
    }

    /** Sort player inventory (mirrors InventoryScreenMixin OI button logic, including fight-mode gating). */
    public static void doOI(Minecraft client, String mode) {
        // Context guard
    ...
    private static void doOST(Minecraft client) {
        if (!(client.gui.screen() instanceof ContainerScreen gcs)) return;
    ...
    public static void handleScreenOpenKeybind(Minecraft client, String mode, boolean isKits) {
        if (client.player == null) return;
        if ("inventory_only".equals(mode)) {
        ...
        net.minecraft.client.gui.screens.Screen parent = client.gui.screen();
        if (isKits) {
            client.gui.setScreen(new KitsScreen(parent, false));
        } else {
            client.gui.setScreen(new VisualInventoryConfigScreen(parent));
        }
    }
}
