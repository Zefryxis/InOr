...
- **Fixed placement landing in the wrong slot within the correct chest.** Even after the routing fix above, an item could still land in the first empty slot that merely *accepted* it (e.g. a generic slot) instead of its actually-designated slot elsewhere in the same chest, as long as it ended up "somewhere valid" in that chest. Placement now ranks every accepting empty slot by specificity (exact item > type > group > generic) and fills the best match — this also improves the regular manual OST/warehouse sort, since it shares the same placement logic.
