...
public class VisualInventoryConfigScreen extends Screen {
    ...
    private void saveToConfig() {
        ...
        config.save();
        System.out.println("[InvOrganizer] Config saved. Slot texts:");
        for (int i = 0; i < 36; i++) {
            if (!slotTexts[i].equals("any")) {
                System.out.println("[InvOrganizer]   slot " + i + " = " + slotTexts[i]);
            }
        }
        for (int i = 0; i < 5; i++) {
            if (!equipTexts[i].equals("any")) {
                System.out.println("[InvOrganizer]   equip " + EQUIP_KEYS[i] + " = " + equipTexts[i]);
            }
        }
    }
    ...
    private String findSelectedGroupRule() {
        ...
        for (int i = 0; i < equipTexts.length && i < equipKeys.length; i++) {
            ...
            System.out.println("[InvOrganizer] Equipment slot " + equipKeys[i] + ": " + rule);
            if (rule != null && !rule.equals("any") && !rule.equals("empty")) {
                // Convert equipment slot rules to group rules
                String groupRule = convertEquipRuleToGroupRule(rule);
                System.out.println("[InvOrganizer] Converted to group rule: " + groupRule);
                if (groupRule != null) {
        ...
        // Debug output
        System.out.println("[InvOrganizer] Group counts: " + groupCounts);
        ...
        System.out.println("[InvOrganizer] Selected group rule: " + bestGroup + " (count: " + maxCount + ")");
        return bestGroup;
    }
