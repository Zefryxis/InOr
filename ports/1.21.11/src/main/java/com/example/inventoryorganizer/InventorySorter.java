...
public class InventorySorter {
    ...
    public static void sortInventory() {
        ...
        for (int e = 0; e < 5; e++) {
            ...
            if (!eqStack.isEmpty()) {
                ...
                System.out.println("[InvOrganizer] Equipment item collected: " + getItemId(eqStack) + " from inv slot " + equipInvSlots[e]);
            }
        }
        ...
        for (ItemStack stack : allItems) {
            ...
            String catName;
            if (cat == CAT_WEAPON) { weapons.add(stack); catName = "weapon"; }
            else if (cat == CAT_TOOL) { tools.add(stack); catName = "tool"; }
            else if (cat == CAT_ARMOR) { armor.add(stack); catName = "armor"; }
            else if (cat == CAT_VALUABLE) { valuables.add(stack); catName = "valuable"; }
            else if (cat == CAT_BLOCK) { blocks.add(stack); catName = "block"; }
            else if (cat == CAT_FOOD) { food.add(stack); catName = "food"; }
            else if (cat == CAT_UTILITY) { utility.add(stack); catName = "utility"; }
            else if (cat == CAT_POTION) { potions.add(stack); catName = "potion"; }
            else if (cat == CAT_ARROW) { arrows.add(stack); catName = "arrow"; }
            else { misc.add(stack); catName = "misc"; }
            System.out.println("[InvOrganizer] Item: " + getItemId(stack) + " x" + stack.getCount() + " -> " + catName);
        }
        ...
        System.out.println("[InvOrganizer] DEBUG: enchOrderArr from config = " + (enchOrderArr != null ? enchOrderArr.length + " items" : "NULL"));
        ...
        if (enchOrderArr != null && enchOrderArr.length > 0) {
            ...
            System.out.println("[InvOrganizer] DEBUG: First 3 enchants: " + enchOrderArr[0] + ", " + (enchOrderArr.length > 1 ? enchOrderArr[1] : "N/A") + ", " + (enchOrderArr.length > 2 ? enchOrderArr[2] : "N/A"));
        }
        ...
        System.out.println("[InvOrganizer] Sort config: criteria=" + java.util.Arrays.toString(finalCriteria)
                ...
                switch (criterion) {
                    ...
                    case "enchant":
                        if (finalEnchOrder != null && finalEnchOrder.length > 0) {
                            ...
                            System.out.println("[InvOrganizer] ENCHANT COMPARE: " + getItemId(a) + " tier=" + enchA + " vs " + getItemId(b) + " tier=" + enchB + " => " + (enchA < enchB ? "A wins" : enchB < enchA ? "B wins" : "tie"));
                            cmp = Integer.compare(enchA, enchB);
                        } else {
                            // Fallback: count enchantments
                            System.out.println("[InvOrganizer] ENCHANT FALLBACK: using count");
        ...
        // Sort potions: type order first (splash/drinkable/lingering), then effect order
        ...
        potions.sort(Comparator.comparingInt((ItemStack s) -> potionTypeFromOrder(s, finalTypeOrder))
                .thenComparingInt((ItemStack s) -> potionTierFromOrder(s, finalPotionOrder))
                .thenComparing(s -> getItemId(s)));
        ...
        System.out.println("[InvOrganizer] hasCustomRules=" + hasCustomRules + " " + debugRules);
        System.out.println("[InvOrganizer] Pool size: " + pool.size() + " items");
        ...
        if (hasCustomRules) {
            ...
            System.out.println("[InvOrganizer] Tier order loaded: " + tierOrder);
            ...
            System.out.println("[InvOrganizer] Tiered slots after sort: " + tieredSlots + " (order: tier 1 first)");
            ...
            for (int i = 0; i < 5; i++) {
                if (!desiredEquip[i].isEmpty()) {
                    System.out.println("[InvOrganizer] DesiredEquip[" + i + "]=" + getItemId(desiredEquip[i]));
                }
            }
        } else {
            // No custom rules - do nothing, keep items where they are
            System.out.println("[InvOrganizer] No custom rules set, skipping sort");
            return;
        }
        ...
        System.out.println(desiredDebug);
        ...
        if (!client.player.playerScreenHandler.getCursorStack().isEmpty()) {
            System.out.println("[InvOrganizer] Aborting sort: cursor is not empty");
            return;
        }
        ...
        for (int targetSlot : orderedSlots) {
            ...
            if (!have.isEmpty()) {
                ...
                if (needsClear) {
                    ...
                    if (emptyDump == -1) {
                        System.out.println("[InvOrganizer] No emptyDump for slot " + targetSlot + ", skipping");
                        continue;
                    }
                    System.out.println("[InvOrganizer] Pre-clearing slot " + targetSlot + " -> " + emptyDump);
            ...
            System.out.println("[InvOrganizer] Swap: want=" + wantId + " target=" + targetSlot + " source=" + sourceSlot);
            ...
            if (!client.player.playerScreenHandler.getCursorStack().isEmpty()) {
                System.out.println("[InvOrganizer] WARNING: cursor not empty after swap, emergency drop");
                ...
                if (!cleared) {
                    System.out.println("[InvOrganizer] ABORT: cursor still not empty, stopping sort");
                    abortSwaps = true;
                }
            }
        }
        ...
        for (int e = 0; e < 5; e++) {
            ...
            if (!desiredEquip[e].isEmpty() && ItemStack.areItemsAndComponentsEqual(equipOriginal[e], desiredEquip[e])) {
                System.out.println("[InvOrganizer] EquipKeep[" + e + "]=" + getItemId(equipOriginal[e]) + " (already correct)");
                continue;
            }
            ...
            if (emptySlot != -1) {
                System.out.println("[InvOrganizer] Unequip[" + e + "]=" + getItemId(equipOriginal[e]) + " to slot " + emptySlot);
        ...
        for (int e = 0; e < 5; e++) {
            ...
            System.out.println("[InvOrganizer] Equip[" + e + "]=" + getItemId(desiredEquip[e])
        ...
        for (int targetSlot = 0; targetSlot < 36; targetSlot++) {
            ...
            if (emptySlot != -1) {
                System.out.println("[InvOrganizer] Moving item from slot " + targetSlot + " to " + emptySlot + " (" + getItemId(current[targetSlot]) + ")");
    ...
    private static void fillBundles(MinecraftClient client, int syncId, OrganizerConfig config, ItemStack[] current) {
        ...
        if (!presetRules.isEmpty()) {
            for (int i = 0; i < 36; i++) {
                ...
                System.out.println("[InvOrganizer] fillBundles: extra bundle found at slot " + i + ", adding to fill plan");
            }
        }
        ...
        for (java.util.Map.Entry<Integer, List<String>> entry : fillPlan.entrySet()) {
            ...
            System.out.println("[InvOrganizer] fillBundles: bundle@" + bundleSlot + " receives " + toBundle.size() + " stacks");
     ...
     */
    private static boolean categoryMatches(ItemStack stack, int targetCat) {
        if (targetCat == CAT_SPLASH_POTION) {
            return getItemId(stack).equals("minecraft:splash_potion");
        }
        return getCategory(stack) == targetCat;
    }
    ...
    static int getCategory(ItemStack stack) {
        ...
        if (id.contains("sword") && !id.contains("_block") && !id.contains("_ore") ||
                id.contains("bow") && !id.contains("cross") ||
        ...
        // Ores, ingots & valuables
        if (id.contains("ore") || id.contains("raw_") || id.contains("ingot") ||
                id.contains("nugget") ||
                (id.contains("diamond") && !id.contains("_block") && !id.contains("_sword") &&
                        !id.contains("_pick") && !id.contains("_axe") && !id.contains("_shovel") &&
                        !id.contains("_hoe") && !id.contains("_helmet") && !id.contains("_chest") &&
                        !id.contains("_leg") && !id.contains("_boot")) ||
                (id.contains("emerald") && !id.contains("_block")) ||
                id.contains("netherite_scrap") || id.contains("amethyst_shard") ||
                id.contains("lapis_lazuli") ||
                (id.contains("coal") && !id.contains("_block") && !id.contains("_ore"))) {
            return CAT_VALUABLE;
        }
    ...
    private static boolean isFood(String id) {
        ...
        return id.contains("apple") || id.contains("bread") || id.contains("beef") ||
                id.contains("pork") || id.contains("chicken") && !id.contains("_egg") ||
                id.contains("mutton") || id.contains("cod") || id.contains("salmon") ||
                id.contains("potato") && !id.contains("poison") || id.contains("carrot") ||
                id.contains("melon_slice") || id.contains("cookie") || id.contains("pie") ||
                id.contains("stew") || id.contains("soup") || id.contains("berr") ||
                id.contains("golden_carrot") || id.contains("cooked_") || id.contains("dried_kelp") &&
                !id.contains("_block") || id.contains("honey_bottle") || id.contains("rabbit") &&
                !id.contains("_hide") && !id.contains("_foot") || id.contains("beetroot") &&
                !id.contains("_seeds") || id.contains("chorus_fruit");
    }
    ...
    private static int getBestEnchantTier(ItemStack stack, String[] enchOrder) {
        try {
            ...
            int bestLevel = 0;
            System.out.println("[InvOrganizer] DEBUG: Checking enchants on " + getItemId(stack));
            for (var entry : enchantments.getEnchantments()) {
                String enchId = entry.getIdAsString();
                int level = enchantments.getLevel(entry);
                System.out.println("[InvOrganizer] DEBUG: Found enchant ID: " + enchId + " level " + level);
                ...
                for (int i = 0; i < enchOrder.length; i++) {
                    if (path.equals(enchOrder[i]) || path.contains(enchOrder[i])) {
                        ...
                        System.out.println("[InvOrganizer] DEBUG: Matched " + path + " at tier " + i + " level " + level + " score=" + score);
                        if (score < bestScore) {
                            bestScore = score;
                            bestEnchant = path;
                            bestLevel = level;
            ...
            System.out.println("[InvOrganizer] Item " + getItemId(stack) + " best enchant: " + bestEnchant + " " + bestLevel + " (score " + bestScore + ")");
            return bestScore;
        } catch (Exception e) {
            System.out.println("[InvOrganizer] ERROR in getBestEnchantTier: " + e.getMessage());
            return 999999;
        }
    }
